...
realUserData = utility;
%----------------------IGCN-----------------------%

% fileID = fopen('results_algebra.txt','w');
for seed = 1:100
    rng(seed-1);
...
for kfold = 1:cross
    ...
    for i=1:size(train_data_set',1)
        clust(:,i) = kmeans(train_data_set',i,'emptyaction','singleton','replicate',5,'Distance','correlation');
        i
    end
    va = evalclusters(train_data_set',clust,'silhouette','Distance','correlation');
    %plot(va);
